...
public class ElasticsearchTests {
    ...
    @Test
    public void testInsertList() {//一次插入多条消息
        discussRepository.saveAll(discussMapper.selectDiscussPosts(101, 0, 100, 0)); //将用户101 从0-100行的消息插入
    ...
    public void testSearchByRepository() {
        SearchQuery searchQuery = new NativeSearchQueryBuilder()
                ...
                .withSort(SortBuilders.fieldSort("type").order(SortOrder.DESC))//按照type排序，SortOrder.DESC倒序排列
                .withSort(SortBuilders.fieldSort("score").order(SortOrder.DESC))//当前面判断相同时，按照score排序
                .withSort(SortBuilders.fieldSort("createTime").order(SortOrder.DESC))
                .withPageable(PageRequest.of(0, 10))// 从0（第一页）开始， 显示10条数据
                .withHighlightFields(
                        new HighlightBuilder.Field("title").preTags("<em>").postTags("</em>"),//高亮显示， "title"中该关键词前后加标签
                        new HighlightBuilder.Field("content").preTags("<em>").postTags("</em>")//高亮显示， "content"中该关键词前后加标签
                ).build();
